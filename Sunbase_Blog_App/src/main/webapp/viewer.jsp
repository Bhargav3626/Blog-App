...
<body>
    ...
    <div class="container">
            ...
            %>
                    <div class="blog-card">
                        ...
                        <p class="blog-content"><%= blog.getContent().substring(0, Math.min(blog.getContent().length(), 100)) + "..." %></p>
                           <a href="ReadMoreBlogServlet?id=<%= blog.getId() %>" target="_blank" class="read-more">Read More</a>
                    </div>
            <% }
            } else { %>
        ...
        <div class="pagination">
            <% if (currentPage > 1) { %>
                <a href="viewer.jsp?page=<%= currentPage - 1 %>">&laquo; Previous</a>
            <% }
            for (int i = 1; i <= totalPages; i++) { %>
                <a href="viewer.jsp?page=<%= i %>" class="<%= i == currentPage ? "active" : "" %>"><%= i %></a>
            <% }
            if (currentPage < totalPages) { %>
                <a href="viewer.jsp?page=<%= currentPage + 1 %>">Next &raquo;</a>
            <% } %>
        </div>
    </div>
</body>
